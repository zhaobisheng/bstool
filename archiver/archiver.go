...
// Walker can walk an archive file and return information
// about each item in the archive.
type WalkerH interface {
	WalkH(archive io.Reader, walkFn WalkFunc) error
}
...
// Walk calls walkFn for each file within the given archive file.
// The archive format is chosen implicitly.
func WalkH(archive io.Reader, walkFn WalkFunc) error {
	wIface, err := ByHeader(archive.(io.ReadSeeker))
	if err != nil {
		return err
	}
	w, ok := wIface.(WalkerH)
