...
// Walk calls walkFn for each visited item in archive.
func (z *Zip) WalkH(archive io.Reader, walkFn WalkFunc) error {
	// bufLen := buf.ReadFrom(archive)
	// inRdrAt, ok := archive.(io.ReaderAt)
	// if !ok {
	// 	return fmt.Errorf("reader must be io.ReaderAt")
	// }
	buf := bytes.NewBuffer([]byte{})
	bufLen, err := io.Copy(buf, archive)
	if err != nil {
		return fmt.Errorf("reader must be io.ReaderAt")
	}
	bufReader := bytes.NewReader(buf.Bytes())
	zr, err := zip.NewReader(bufReader, bufLen)
	if err != nil {
		return fmt.Errorf("opening zip reader: %v", err)
	}
...
func (*Zip) Match(file io.ReadSeeker) (bool, error) {
	...
	if bytes.Equal(buf[:2], []byte("MZ")) {
		pef, _ := pe.NewFile(file.(io.ReaderAt))
		lastsec := pef.Sections[int(pef.FileHeader.NumberOfSections-1)]
		offset := lastsec.Offset + lastsec.Size
